...
test("context-compile CLI parses structured anchors and budget", () => {
  assert.deepEqual(parseContextCompileArgs(["repo", "--task", "AuthService", "--anchor-file", "src/auth.ts", "--changed-path", "src/auth.ts", "--budget-items", "3", "--json"]).input, {
    task: "AuthService",
