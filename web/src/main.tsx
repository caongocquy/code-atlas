...
function Retrieval({ onGraph }: { onGraph: (chunk: Chunk) => void }): ReactElement {
  ...
  const run = async (): Promise<void> => { setError(null); setLoading(true); try { setInspection(await api<Inspection>("/api/retrieval/inspect", { method: "POST", body: JSON.stringify({ query, ...options }) })); } catch (requestError) { setError(requestError instanceof Error ? requestError.message : String(requestError)); } finally { setLoading(false); } };
