...
- `task_exact_resolution` is emitted only by an explicit Phase15B authority check, not by `resolveGraphEntity(...).status === "resolved"`. The check accepts only `exact_qualified_name`, `exact_symbol_name`, or `exact_normalized_token`; for a file-qualified task target it additionally requires exact normalized repository-relative file equality and exact or normalized-equal symbol equality. `file_path_context`, `prefix`, `lexical_relevance`, and any result whose exactness cannot be proven from the query/entity pair are forbidden. A uniquely mapped retrieval result may still be an exact-addressable subject, but remains retrieval/supporting/optional and budgeted.
...
- [ ] Include authority-boundary cases: an exact symbol-name task query and an exact qualified-symbol query emit `task_exact_resolution` and become required; a uniquely resolved prefix, a uniquely resolved lexical substring, and a file-qualified prefix symbol emit no `task_exact_resolution` and are not required; an exact file-qualified symbol emits it only when both normalized repository-relative path and exact/normalized-equal symbol match.
- [ ] Run the focused collection test; expected: FAIL because the collector is not exported.
- [ ] Implement authority order: validate explicit anchors; resolve files against the already canonical `repositoryPath` and repository-relative existence/index metadata; resolve direct task targets via `resolveGraphEntity`, then pass the query and result through a small explicit-semantics helper such as `isAuthoritativeTaskResolution(query, resolution): boolean`. The helper may emit `task_exact_resolution` only for a unique result whose winning match reason is `exact_qualified_name`, `exact_symbol_name`, or `exact_normalized_token`; for file-qualified queries it must also prove exact normalized repository-relative file equality and exact or normalized-equal symbol equality. Do not use `status === "resolved"` or `file_path_context` alone as proof, and do not change the global resolver. Preserve ambiguous/not_found and unique prefix/lexical results as non-authoritative diagnostics/retrieval evidence. Process `explicit_changed_path` separately and require current readable-file proof before creating its exact required file subject.
...
- [ ] Write failing table-driven tests for required explicit/direct-task-exact/changed candidates, supporting direct neighbors/corroborated retrieval/affected tests, optional weak or one-source candidates, ambiguous non-required candidates, and exact-over-fuzzy dominance. Cover all authority boundaries: exact symbol name, exact qualified symbol, and exact file-qualified symbol (exact or normalized-equal path/symbol) carry `task_exact_resolution` and are required; unique prefix, unique lexical substring, and file-qualified prefix results—even when the resolver reports `status: "resolved"`—carry no `task_exact_resolution` and are not required. Prove that a lexical fuzzy candidate mapped uniquely to an exact symbol has `exact=true` but remains supporting/optional and budgeted.
- [ ] Run:
  ```bash
  node --import tsx/esm --test test/phase15b-ranker.test.ts
  ```
  Expected: FAIL because the ranker is not implemented.
- [ ] Implement priority as a rule-first decision: only explicit anchors, explicit changed paths, and direct task resolution carrying the helper-proven `task_exact_resolution` evidence are required; exactness alone never promotes a candidate. Direct graph neighbors, corroborated retrieval, and directly affected tests are supporting; weak/one-source evidence is optional; unresolved/ambiguous evidence cannot be required. Retrieval candidates remain budgeted even when their subject is exactly addressable.
