...
**Spec:** [Phase14B revised design](<HOME>/code-atlas/docs/superpowers/specs/2026-09-07-phase14b-multilanguage-resolver-typeenvironment-v2-design-revised.md), §§22–27, 30, 37–51.
...
```bash
git add src/core/indexing/index-pipeline.service.ts src/core/indexing/filesystem-change-detector.ts src/core/indexing/indexing.types.ts test/phase14b-pipeline-scope.test.ts test/phase14a-indexing.test.ts test/phase14a-single-parse.test.ts
...
**Interfaces:**
- Produces `rebindCandidateEdges(previousGraph: CodeGraph, candidateSymbols: readonly SymbolIdentity[], resolutionVersion: string): GraphEdge[]` that returns only edges whose endpoint logical identities resolve uniquely in the candidate.
...
```bash
git add src/core/indexing/index-pipeline.service.ts src/core/graph/build-file-updates.ts src/core/indexing/invalidation-planner.ts test/phase14b-incremental-equivalence.test.ts test/phase14a-equivalence.test.ts test/phase14a-invalidation.test.ts
...
**Files:**
- Modify: `src/core/indexing/index-pipeline.service.ts`
- Modify: `src/storage/atlas/atlas.store.ts` only at the existing candidate-copy/publish call boundary
...
Use the independent resolution version in cache planning and edge provenance, rebuild the transient environment from cached facts, clear candidate semantic rows before writing current results, and rely on the existing candidate transaction so active state remains unchanged on any failure.
...
```bash
git add src/core/indexing/index-pipeline.service.ts src/storage/atlas/atlas.store.ts test/phase14b-resolution-version.test.ts test/phase14b-candidate-failure.test.ts test/phase14a-failures.test.ts test/phase14a-races.test.ts
...
- Modify: `test/helpers/phase14b-facts.ts` (extend the shared fact fixture helpers with repository/language/conformance builders)
- Modify: `test/phase14a-equivalence.test.ts` only if its normalization must include declared Phase14B provenance fields

**Interfaces:**
- `runPhase14bFixture(name: string, options?: { memoMode?: "cold" | "warm"; parallel?: boolean }): Promise<FixtureResult>` returns decisions, normalized edges, diagnostics, counters, and incompleteness.
...
- `runPackedMcpInitialize(cliPath: string): Promise<{ protocolVersion: string; serverName: string }>` spawns `node cliPath mcp`, sends one MCP `initialize` request, verifies the JSON response, and closes the child.
...
```bash
git add test/phase14b-conformance.test.ts test/phase14b-packed-mcp-smoke.test.ts test/helpers/phase14b-fixtures.ts test/phase14a-equivalence.test.ts
