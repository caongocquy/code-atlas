...
- Follow-up relaxes fact ID validation to accept declared non-negative IDs such as `scope:0` while rejecting malformed forms.
- Final ID fix rejects non-canonical leading-zero IDs such as `scope:00` and `scope:01`.
...
- Tests cover hit including `scope:0`, rejection of leading-zero IDs, canonical round-trip, every miss reason including schema/language mismatch, actual cross-path/cross-repository reuse, direct SQLite corruption, repair, sequential writer immutability, and schema contents.
