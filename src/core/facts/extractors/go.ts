...
function extractGoTreeFacts(parsed: ParsedSource | undefined, input: LanguageFactExtractorInput): FactExtractionOutcome {
  try {
    ...
    const symbol = (node: Parser.SyntaxNode, kind: ParsedSymbolFact["kind"], name: string, scopeId = scopeStack.at(-1)): ParsedSymbolFact => {
      const value = { localId: next("symbol"), name, kind, range: range(node), scopeId, declaredQualifiedName: name };
    ...
    const callableType = (node: Parser.SyntaxNode): string | undefined => typeName(field(node, "type"));
    const visit = (node: Parser.SyntaxNode): void => {
      ...
      if (node.type === "package_clause") {
        const name = node.namedChildren[0]; if (name) modules.push({ localId: next("module"), name: name.text, moduleKind: "package", exported: true, range: range(node) });
        ...
        if (name) { declared = symbol(node, "method" === "method" && receiver ? "method" : "function", name); callableStack.push(declared); }
        ...
        const name = field(node, "name")?.text; if (name && typeStack.at(-1)) { declared = symbol(node, "method", name, scopeStack.at(-1)); members.push({ localId: next("member"), ownerSymbolId: typeStack.at(-1)!.localId, memberName: name, memberKind: "method", access: "instance", range: range(node) }); }
      ...
      } else if (node.type === "parameter_declaration" && callableStack.at(-1)) {
        ...
        const operand = field(node, "operand"), memberName = field(node, "field"); if (operand && memberName) members.push({ localId: next("member"), receiverId: expression(operand, operand.type === "selector_expression" ? "member" : "identifier").localId, memberName: memberName.text, memberKind: "method", access: "instance", range: range(node) });
      } else if (node.type === "call_expression") {
        const fn = field(node, "function") ?? node.namedChildren[0]; if (fn) calls.push({ localId: next("call"), calleeText: fn.text, callerId: callableStack.at(-1)?.localId, scopeId: scopeStack.at(-1), range: range(node) });
    ...
    const methodReceivers = new Map<string, ParsedSymbolFact>();
    for (const parameter of parameters.filter((item) => item.receiverKind === "go_receiver")) methodReceivers.set(parameter.typeText?.replace("*", "") ?? "", symbols.find((item) => item.localId === parameter.ownerSymbolId)!);
    const interfaces = symbols.filter((item) => item.kind === "interface");
    for (const concrete of symbols.filter((item) => item.kind === "method")) { const receiver = [...methodReceivers.entries()].find(([, method]) => method.localId === concrete.localId)?.[0]; if (!receiver) continue; for (const iface of interfaces) if (members.some((item) => item.ownerSymbolId === iface.localId && item.memberName === concrete.name)) implementations.push({ localId: next("implementation"), subjectId: symbols.find((item) => item.name === receiver)?.localId ?? concrete.localId, targetName: iface.name, relationKind: "interface", range: concrete.range }); }
