...
const navigationMember = (node: Parser.SyntaxNode | undefined) => node?.type === "navigation_suffix"
...
function extractKotlinTreeFacts(parsed: ParsedSource | undefined, input: LanguageFactExtractorInput): FactExtractionOutcome {
  try {
    ...
    const visit = (node: Parser.SyntaxNode): void => {
      ...
      const syntaxNode = syntax.observe(node, callableStack.at(-1)?.localId ?? typeStack.at(-1)?.localId, scopeStack.at(-1));
      ...
      if (isType) { const name = node.namedChildren.find((child) => child.type === "type_identifier")?.text; if (name) { const isInterface = node.children.some((child) => child.type === "interface"); declared = symbol(node, isInterface ? "interface" : "class", name); typeStack.push(declared); if (node.type === "object_declaration") declared.name = name; const primary = node.namedChildren.find((child) => child.type === "primary_constructor"); if (node.type === "class_declaration" && primary) constructors.push({ localId: next("constructor"), ownerSymbolId: declared.localId, constructedTypeName: name, range: range(primary) }); } const specs = node.namedChildren.filter((child) => child.type === "delegation_specifier"); for (const spec of specs) { const target = spec.namedChildren.find((child) => ["user_type", "constructor_invocation"].includes(child.type)); const targetName = target?.namedChildren[0]?.text ?? target?.text; if (declared && target && targetName) (target.type === "user_type" ? implementations : inheritances).push({ localId: next(target.type === "user_type" ? "implementation" : "inheritance"), subjectId: declared.localId, targetName, relationKind: target.type === "user_type" ? "implements" : "extends", range: range(target) } as ImplementationFact & InheritanceFact); } }
      ...
      if (node.type === "class_parameter") { const name = node.namedChildren.find((child) => child.type === "simple_identifier"), type = typeNode(node); const owner = typeStack.at(-1); if (name && owner) { const binding = bind(node, name.text, "field", owner.scopeId); const member = symbol(node, "variable", name.text); member.scopeId = owner.scopeId; addType(member.localId, type); const parameterId = next("parameter"); parameters.push({ localId: parameterId, ownerSymbolId: owner.localId, name: name.text, bindingId: binding.localId, typeText: type?.text, index: parameters.filter((item) => item.ownerSymbolId === owner.localId).length, range: range(node) }); syntax.linkOwner(node, member.localId); syntax.linkFact(node, parameterId); } }
      if (node.type === "function_declaration") { const parameterContainer = node.namedChildren.find((child) => child.type === "function_value_parameters"); const owner = callableStack.at(-1); for (const parameter of parameterContainer?.namedChildren ?? []) { const name = parameter.namedChildren.find((child) => child.type === "simple_identifier"); const type = typeNode(parameter); if (name && owner) { const binding = bind(parameter, name.text, "parameter"); const parameterId = next("parameter"); parameters.push({ localId: parameterId, ownerSymbolId: owner.localId, name: name.text, bindingId: binding.localId, typeText: type?.text, index: parameters.filter((item) => item.ownerSymbolId === owner.localId).length, range: range(parameter) }); syntax.linkFact(parameter, parameterId); const parameterIndex = parameterContainer?.namedChildren.indexOf(parameter) ?? -1; const modifiers = parameterIndex > 0 ? parameterContainer?.namedChildren[parameterIndex - 1] : undefined; for (const annotation of modifiers?.descendantsOfType("annotation") ?? []) syntax.linkFact(annotation, parameterId); addType(binding.localId, type); } } }
      ...
      if (node.type === "navigation_expression") { const receiver = node.namedChildren[0], memberName = navigationMember(node.namedChildren.at(-1)); if (receiver && memberName && receiver !== memberName) { const value = expression(receiver, "member"); const callSite = node.parent?.type === "call_expression" ? callSites.at(-1) : undefined; members.push({ localId: callSite?.localId ?? next("member"), receiverId: value.localId, memberName: memberName.text, memberKind: "method", access: "instance", range: range(node) }); } }
      ...
      if (node.type === "simple_identifier" && node.parent?.type !== "function_declaration") { const reference = { localId: next("reference"), name: node.text, scopeId: scopeStack.at(-1), range: range(node) }; references.push(reference); syntax.linkFact(node, reference.localId); }
    ...
    const facts: ParsedFactsBlob = { factsSchemaVersion: input.factsSchemaVersion, factsVersion: input.factsVersion, contentHash: input.contentHash, language: "kotlin", parserIdentity: { language: parsed.adapter.language, ...parsed.adapter.metadata }, parseStatus, parserDiagnostics: [...new Set(diagnostics)], symbols, containmentScopes, imports, exports: [], references, callSites, bindingSeeds, declaredTypeAnnotations, expressions, members, assignments, parameters, returns, constructors, inheritances, implementations, aliases, modules, namespaces: [], frameworkSyntax: syntax.finish(parseStatus === "complete", parsed.tree.rootNode) };
