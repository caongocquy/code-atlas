  ...
  try {
    ...
    const visit = (node: Parser.SyntaxNode): void => {
      ...
      if (node.type === "field_declaration") {
        ...
        if (name && owner) { const member = symbol(value ?? node, "variable", name); member.scopeId = owner.scopeId; bind(value ?? node, name, "field", owner.scopeId); typeAnnotation(member.localId, field(node, "type")); }
      }
      if (node.type === "formal_parameter") { const name = text(field(node, "name")), owner = callableStack.at(-1); if (name && owner) { const binding = bind(node, name, "parameter"); parameters.push({ localId: next("parameter"), ownerSymbolId: owner.localId, name, bindingId: binding.localId, typeText: text(field(node, "type")), index: parameters.filter((item) => item.ownerSymbolId === owner.localId).length, range: range(node) }); typeAnnotation(binding.localId, field(node, "type")); } }
      ...
      if (node.type === "identifier" && node.parent && ["method_invocation", "field_access"].includes(node.parent.type) === false) references.push({ localId: next("reference"), name: node.text, scopeId: scopeStack.at(-1), range: range(node) });
