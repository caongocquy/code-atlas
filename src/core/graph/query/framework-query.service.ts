...
export function projectFrameworkGraph(graph: CodeGraph, snapshot: FrameworkSnapshot | undefined, expectedFrameworkVersion?: string): FrameworkQueryProjection {
...
export function queryFrameworkProjection(projection: FrameworkQueryProjection, subject: FrameworkSubjectRef, depth: number): FrameworkQueryProjection {
  ...
  const languageNodes = new Map(projection.nodes.filter((node): node is Extract<FrameworkQueryNode, { kind: "language" }> => node.kind === "language").map((node) => [subjectKey({ kind: "language", nodeId: node.node.id }), node]));
  const frameworkNodes = new Map(projection.nodes.filter((node): node is Extract<FrameworkQueryNode, { kind: "framework" }> => node.kind === "framework").map((node) => [subjectKey({ kind: "framework", entity: node.entity.ref }), node]));
