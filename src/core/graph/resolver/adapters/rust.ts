...
export function normalizeRustFacts(facts: ParsedFactsBlob, context: AdapterContext): SemanticEvidenceBatch {
  ...
  for (const item of facts.members) {
    ...
    result.diagnostics = [...result.diagnostics, { ...base("member", item.localId, item.range), code: "runtime_dispatch", message: `Rust member dispatch is not uniquely observed for ${item.memberName}`, sourceUnit: unit }];
  }
