...
export function normalizePythonFacts(facts: ParsedFactsBlob, context: AdapterContext): SemanticEvidenceBatch {
  ...
  for (const item of facts.bindingSeeds) if (item.bindingKind === "function") {
    const symbol = facts.symbols.find((candidate) => candidate.kind === "function" && candidate.name === item.name && candidate.range.startLine === item.range.startLine);
    if (symbol) typeByBinding.set(item.localId, { kind: "known", symbol: symbols.get(symbol.localId)! });
  }
  ...
  for (const item of facts.callSites) if (["eval", "exec", "compile", "__import__", "setattr", "getattr", "delattr", "globals", "locals"].includes(item.calleeText)) result.diagnostics = [...result.diagnostics, { sourceUnit: unit, range: item.range, code: "compiler_semantics_required", message: `Runtime-dependent Python construct: ${item.calleeText}` }];
