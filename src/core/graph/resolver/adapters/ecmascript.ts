...
export function normalizeEcmascriptFacts(facts: ParsedFactsBlob, context: AdapterContext): SemanticEvidenceBatch {
  ...
  const scopeOf = (localId: string | undefined): ScopeIdentity => {
    const scope = localId ? scopes.get(localId as FactLocalId) : undefined;
  ...
  const typeBySymbol = new Map<string, TypeRef>();
  for (const annotation of facts.declaredTypeAnnotations) {
    const type = typeOf(annotation.text);
    if (!type) continue;
    if (bindingById.has(annotation.ownerId)) typeByBinding.set(annotation.ownerId, type);
    if (symbols.has(annotation.ownerId)) typeBySymbol.set(annotation.ownerId, type);
  }
  ...
  const parentScope = (localId: string | undefined): string | undefined => localId ? scopes.get(localId as FactLocalId)?.parentId : undefined;
  const findBinding = (name: string | undefined, localId: string | undefined) => {
  ...
  for (const item of facts.declaredTypeAnnotations) {
    ...
    if (!type) continue;
    result.typeAnnotations = [...result.typeAnnotations, { ...base("type", item.localId, item.range), kind: "type_annotation", subjectLocalId: item.ownerId, type }];
    result.bindings = result.bindings.map((binding) => binding.bindingId === item.ownerId ? { ...binding, declaredType: type } : binding);
  }
  ...
  for (const item of facts.assignments) {
    ...
    const sourceBinding = findBinding(item.sourceName, undefined);
  ...
  for (const item of facts.returns) result.returns = [...result.returns, { ...base("return", item.localId, item.range), kind: "return", callable: symbols.get(item.ownerSymbolId)!, expression: item.expressionId ? { sourceUnit: unit, localId: item.expressionId } : undefined, type: typeBySymbol.get(item.ownerSymbolId) }];
  ...
  for (const item of facts.members) {
    ...
    result.diagnostics = [...result.diagnostics, { sourceUnit: unit, range: item.range, code: "receiver_type_unknown", message: `Cannot resolve receiver for ${item.memberName}` }];
    }
  }
  ...
  if (facts.parseStatus !== "complete" || facts.parserDiagnostics.length > 0) {
    const message = facts.parserDiagnostics.join(", ") || "Tree-sitter reported an uncertain parse";
    result.diagnostics = [...result.diagnostics, { sourceUnit: unit, code: "parse_uncertain", message }];
    result.diagnostics = [...result.diagnostics, { sourceUnit: unit, code: "language_capability_unsupported", message: "Partial ECMAScript facts are not authoritative" }];
  }
  return result;
}
