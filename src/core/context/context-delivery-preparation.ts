...
import { canonicalProjectionIdentity, getWorkspaceIdentity } from "./context-identity.js";
...
function reliability(status: Awaited<ReturnType<typeof getRepositoryStatusReadOnly>> | undefined): unknown {
  return { source: "current-codeatlas-state", mayBeIncomplete: status ? status.graph.status === "stale" || status.graph.resolutionCoverage.mayBeIncomplete : true, graph: status?.graph.status ?? "unavailable" };
}
...
function metrics(result: ContextAwareReadResult, content: string) {
  const returnedBytes = Buffer.byteLength(result.content ?? (result.delta ? JSON.stringify(result.delta) : ""));
...
export async function prepareContextAwareRead(repoPath: string, request: ContextAwareReadRequest, store?: ContextStore, storeError?: unknown): Promise<PreparedContextAwareRead> {
  ...
  if (!store) {
    ...
    const result = { mode: "rehydrate" as const, receipt, current: { contentIdentity: currentIdentity, reliability: receipt.reliability }, content, reason: `context_database_unavailable: ${storeError instanceof Error ? storeError.message : String(storeError ?? "unavailable")}` };
    return { session, receipt, snapshot: createDeliveredSnapshot({ snapshotId: receipt.snapshotId, receiptId: receipt.receiptId, subjectIdentity, projectionIdentity }, content), result, metrics: metrics(result, content) };
  }
  ...
  const delta = previous && previousSnapshot && previous.deliveredContentIdentity !== currentIdentity ? createExactDelta(previousSnapshot.content, content) : undefined;
  ...
  return { session, receipt, snapshot, result, metrics: metrics(result, content) };
}
