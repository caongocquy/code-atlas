...
async function main(): Promise<void> {
  ...
  if (!command || !value) {
    throw new Error(
      [
        "Usage:",
        "pnpm exec tsx src/query-graph.ts callers <symbol>",
        "pnpm exec tsx src/query-graph.ts callees <symbol>",
        "pnpm exec tsx src/query-graph.ts imports <file>",
        "pnpm exec tsx src/query-graph.ts imported-by <file>",
      ].join("\n"),
    );
  }
    ...
    if (graph.nodes.length === 0) {
      throw new Error(
        [
          ...
          "Run:",
          "pnpm exec tsx src/index-graph.ts .",
        ].join("\n"),
      );
    }
